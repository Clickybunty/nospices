...
fs.readFile('transformedZutatenIDs.json', 'utf8', (err, data) => {
  if (err) {
  ...
  transformedData.forEach((item) => {
    ...
    connection.query(
      'INSERT INTO zutaten (id, ids) VALUES (?, ?)',
      [id, JSON.stringify(ids)], // IDs müssen als JSON-String gespeichert werden
      (err, result) => {
        if (err) {
